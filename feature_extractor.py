...
def get_num_subdomains(url):
    """www شمرده نمی‌شه چون یه alias معموله، نه یه ساب‌دامین واقعی"""
    if pd.isna(url):
        return 0
    try:
...
def get_num_dots(url):
    """www رو قبل از شمردن نقطه‌ها حذف می‌کنیم، چون www یه alias معموله نه سیگنال واقعی"""
    if pd.isna(url):
        return 0
    try:
...
def get_has_login_keywords(url):
    """login تو hostname = مشکوک (مثل login.ru تو تست فیشینگ)"""
    if pd.isna(url):
        return 0
    try:
...
def get_has_login_path(url):
    """login تو path = معمولاً عادیه (endpoint واقعی مثل /auth/login)"""
    if pd.isna(url):
        return 0
    try:
...
def get_hostname_hyphens(url):
    """هایفن تو دامنه = تلاش برای جعل برند (paypal-secure.com)"""
    if pd.isna(url):
        return 0
    try:
...
def get_path_hyphens(url):
    """هایفن تو path = معمولاً فقط SEO slug عادیه (بلاگ‌پست‌ها)"""
    if pd.isna(url):
        return 0
    try:
...
if __name__ == '__main__':
    ...
    df['has_login_path']     = df['url'].apply(get_has_login_path)
    df['hostname_hyphens']   = df['url'].apply(get_hostname_hyphens)
    df['path_hyphens']       = df['url'].apply(get_path_hyphens)
